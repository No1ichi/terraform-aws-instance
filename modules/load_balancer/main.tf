...
resource "aws_lb_target_group" "pp_alb_target_group" {
    name     = "tg_webserver"
    port     = 80
    protocol = "HTTP"
...
resource "aws_lb_listener" "pp_alb_listener" {
    load_balancer_arn = aws_lb.pp_alb.arn
    port              = 443
    protocol          = "HTTPS"
    ...
    default_action {
        type             = "forward"
        target_group_arn = aws_lb_target_group.pp_alb_target_group.arn
    }
